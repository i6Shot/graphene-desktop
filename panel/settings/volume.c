...
static void graphene_volume_icon_class_init(GrapheneVolumeIconClass *klass)
{
  ...
  gobjectClass->dispose = graphene_volume_icon_dispose;
}
...
static void graphene_volume_slider_dispose(GObject *self_);
...
{
  GObjectClass *gobjectClass = G_OBJECT_CLASS(klass);
  gobjectClass->dispose = graphene_volume_slider_dispose;
}
...
static void graphene_volume_slider_dispose(GObject *self_)
{
  ...
  g_clear_object(&self->volumeControl);
  G_OBJECT_CLASS(graphene_volume_slider_parent_class)->dispose(self_);
}
