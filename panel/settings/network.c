...
G_DEFINE_TYPE(GrapheneNetworkIcon, graphene_network_icon, GTK_TYPE_IMAGE)


GrapheneNetworkIcon* graphene_network_icon_new(void)
{
  return GRAPHENE_NETWORK_ICON(g_object_new(GRAPHENE_TYPE_NETWORK_ICON, NULL));
}
...
static void graphene_network_icon_init(GrapheneNetworkIcon *self)
{
  ...
  g_signal_connect_swapped(self->networkControl, "update", G_CALLBACK(icon_on_update), self);
...
static void icon_on_update(GrapheneNetworkIcon *self, GrapheneNetworkControl *networkControl)
{
  gtk_image_set_from_icon_name(GTK_IMAGE(self), graphene_network_control_get_icon_name(networkControl), GTK_ICON_SIZE_MENU);
}